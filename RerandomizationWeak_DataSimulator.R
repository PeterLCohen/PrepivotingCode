...
nrand = 500 #1000 # The number of simulations to run
...
SdT = 10 # Standard deviation for treated potential outcomes
...
# Vectors for generating outcomes as noisy linear model of covariates
betaT = c(1,2,3)*.2 # This one is for treated outcomes
betaC = c(-8,-5, -3)*.8 # This one is for control outcomes
...
while(i < nrand)
{
  ...
  Yttemp = X%*%betaT - (rexp(N, 1/SdT) - SdT)
  Yc = X%*%betaC - (rexp(N, 1/SdC) - SdC)
  Yt = Yttemp - mean(Yttemp- Yc) # Center to enforce Neyman's weak null
  ...
  if(mah <= tol) # if the Mahalanobis distance doesn't exceed the tolerance then the treatment allocation is used; otherwise we re-draw a new treatment allocation
  {
    ...
    ZZ = mvrnorm(ngauss, rep(0, 4), Vhat)
    ...
    statpre[i] = mean(abs(ZZ[,1]) <= statabs[i] & pp <= tol)/mean(pp <= tol)
...
#########                    Save the results                       ############
################################################################################
save(file = "rerandWeak", pvalabs, pvalstu, pvalpre, statabs, statstu, statpre, refdistabs, refdiststu, refdistpre)
